...
새 기능을 추가할 때는 반드시 아래 순서를 따른다. 순서를 건너뛰는 것은 불완전한 작업으로 간주한다.

```
1단계. docs/PRD.md 설계
   └─ 기능 명세 테이블에 항목 추가 (설명·우선순위·역할별 권한·상태 ⬜)
   └─ 필요 시 데이터 모델, 화면 흐름, 권한 정리 테이블 업데이트
   └─ 버전 및 최종 수정일 갱신

2단계. CHECKLIST.md 등록
   └─ 해당 섹션(API / 웹 / 모바일)에 구현 항목을 ⬜ 상태로 추가
   └─ 마지막 갱신 날짜 업데이트
...
4단계. 완료 처리
   └─ CHECKLIST.md 상태 ⬜ → ✅
   └─ docs/changelog/YYYY-MM-DD.md 에 변경사항 기록
   └─ 위 문서 변경사항을 같은 Git 커밋에 포함
```

> **핵심 원칙**: 코드보다 문서가 먼저다. PRD 설계 없이 구현을 시작하지 않는다.
...
| 변경 유형 | 갱신 대상 |
|-----------|-----------|
| **새 기능 기획** | `docs/PRD.md` → `CHECKLIST.md` 순서로 등록 (위 절차 참고) |
...
| 미구현 항목 발견 / 추가 | `docs/PRD.md` 설계 후 `CHECKLIST.md` 에 새 행 추가 |
